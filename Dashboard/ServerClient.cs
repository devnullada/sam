...
public class ServerClient : IDisposable
{
    ...
    public async Task<List<ServiceStatus>> GetServices()
    {
        try
        {
            ...
            return JsonSerializer.Deserialize<List<ServiceStatus>>(json, _jsonOpts) ?? [];
        }
        catch { return []; }
    }
    ...
    public async Task<string?> GetConfigPath()
    {
        try
        {
            var json = await _http.GetStringAsync("/config-path");
    ...
    public async Task<ServiceEntry?> GetServiceConfig(string name)
    {
        try
        {
            var json = await _http.GetStringAsync($"/services/{name}/config");
            return JsonSerializer.Deserialize<ServiceEntry>(json, _jsonOpts);
        }
        catch { return null; }
    }

    public async Task<bool> UpdateServiceConfig(string name, ServiceEntry entry)
    {
        try
        {
            var resp = await _http.PutAsJsonAsync($"/services/{name}/config", entry, _jsonOpts);
            return resp.IsSuccessStatusCode;
        }
        catch { return false; }
    }

    public async Task<bool> CreateService(ServiceEntry entry)
    {
        try
        {
            var resp = await _http.PostAsJsonAsync("/services", entry, _jsonOpts);
            return resp.IsSuccessStatusCode;
        }
        catch { return false; }
    }

    public async Task<bool> DeleteService(string name)
    {
        try
        {
            var resp = await _http.DeleteAsync($"/services/{name}");
            return resp.IsSuccessStatusCode;
        }
        catch { return false; }
    }

    public async Task ClearOutput(string name)
    {
        try { await _http.PostAsync($"/services/{name}/output/clear", null); } catch { }
    }
